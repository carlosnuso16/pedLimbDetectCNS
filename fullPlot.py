...
# --- 1. CONFIGURATION ---
# --- IMPORTANT: Point this to your NEWLY-CREATED tfrecords folder ---
tfFOLDERS = '/mnt/SeagateC25_stora/pedLimbDetectCNS/tfrecords' # <--- !!FIX ME!! Make sure this is correct
TRAIN_TFRECORD_DIR = os.path.join(tfFOLDERS, "train")
MAX_PLOTS = 10 # We'll plot the first 10 segments with movement

# These constants must match your TFRecord data
SF_TARGET = 128
ANOT_TARGET_FREQ = 2
NUM_CHANNELS = 2
...
# --- 2. TFRecord Parsing (The working float32 parser) ---
def parse_tfrecord(example_proto):
    ...
    # 1. Decode signals as float32
    signals = tf.io.decode_raw(parsed_example['signals'], tf.float32)
    
    # 2. Reshape and Transpose to (Time, Channels)
...
def plot_segment(signal_segment, annot_segment, segment_index):
    ...
    fig.suptitle(f'Alignment Check (from TFRecord): Segment {segment_index} (30 seconds)', fontsize=16)
...
def main():
    ...
    if not train_tfrecords:
        raise FileNotFoundError(f"No files found at {TRAIN_TFRECORD_DIR}. Did you run tfRecordMake_FIXED.py yet?")
