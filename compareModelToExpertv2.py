...
def multiple_lines_plot_masks_channel(subject, trace, PLM_expert, y, fs, normalize_signal=False, minutes_per_row=10, row_height=30):
    ...
    # --- OFFSET SETTING ---
    # Increase this to separate the channels more
    CHANNEL_OFFSET = 20
    ...
    for ri in range(nrow):
        ...
        # LAT (Channel 1) - Offset by CHANNEL_OFFSET
        ax.plot(trace[row_ids[ri], 1] + 1.5*2*ri * row_height + CHANNEL_OFFSET, c='k', lw=0.5, alpha=0.7)
        ...
        # Updated to match the new CHANNEL_OFFSET
        ticklocs = [ri * row_height * 3, ri * row_height * 3 + CHANNEL_OFFSET] 
...
def main():
    ...
    plots_generated = 0
    MAX_PLOTS = 1 # Stop after finding 1 good example (the first patient)

    # Loop through files
    for file_path in tqdm(files_to_process, desc="Scanning Subjects"):
        if plots_generated >= MAX_PLOTS:
            print(f"Generated plots for {plots_generated} patient(s). Stopping.")
            break
        ...
        # 5. GLOBAL NORMALIZATION & CLIPPING
        print(f"Normalizing and clipping signals for {subject_name}...")
        # Normalize based on the IQR of the WHOLE night (more robust)
        # This makes sure a 'quiet' 15-min chunk doesn't get blown up to look like noise
        median = np.median(full_signals, axis=0)
        iqr = np.quantile(full_signals, 0.75, axis=0) - np.quantile(full_signals, 0.25, axis=0)
        
        # Prevent divide by zero
        iqr[iqr == 0] = 1.0 
        
        full_signals = (full_signals - median) / iqr
        
        # CROP AMPLITUDES (Clipping)
        # Since we normalized by IQR, values are roughly "standard deviations". 
        # +/- 20 is a very safe range that keeps signal but cuts off massive artifacts.
        CLIP_THRESHOLD = 20.0 
        full_signals = np.clip(full_signals, -CLIP_THRESHOLD, CLIP_THRESHOLD)

        # 6. PLOTTING (Full Night Stacked)
        print(f"Generating full night stacked plot (15 mins per row)...")
        
        # We pass the ENTIRE night's data here.
        # 'minutes_per_row=15' tells the function to create a new row every 15 mins.
        # This will create one very tall image with all rows stacked.
        fig = multiple_lines_plot_masks_channel(
            subject=f"{subject_name} (Full Night)",
            trace=full_signals,
            PLM_expert=full_annotations_128hz,
            y=full_predictions_128hz,
            fs=SF_TARGET,
            normalize_signal=False, # We already normalized globally above
            minutes_per_row=15 
        )
        
        output_filename = f"full_night_stacked_{subject_name}.png"
        fig.savefig(output_filename, dpi=150, bbox_inches='tight')
        print(f"Saved full night plot to: {output_filename}")
