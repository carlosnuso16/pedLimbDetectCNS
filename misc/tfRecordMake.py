...
def save_all_segments_to_tfrecord(folder, tfrecord_path):
    ...
    with h5py.File(h5_file, 'r') as f:
        ...
        # after computing num_segments and valid_annotations
        expected_anot_len = num_segments * SEGMENT_LEN_SEC * ANOT_TARGET_FREQ  # num_segments * 60
        if len(valid_annotations) < expected_anot_len:
            # pad with zeros (or choose appropriate pad value)
            pad_len = expected_anot_len - len(valid_annotations)
            valid_annotations = np.concatenate([valid_annotations, np.zeros(pad_len, dtype=valid_annotations.dtype)])
        elif len(valid_annotations) > expected_anot_len:
            # trim to the expected length
            valid_annotations = valid_annotations[:expected_anot_len]

        # Optional duration check (prints for debugging)
        signal_duration = signalShape / SF_TARGET
        anot_duration = len(valid_annotations) / ANOT_TARGET_FREQ
        if abs(signal_duration - anot_duration) > 0.5:  # allow half-second mismatch
            print(f"Warning: duration mismatch for {subject_id}: signal {signal_duration}s vs anot {anot_duration}s")
